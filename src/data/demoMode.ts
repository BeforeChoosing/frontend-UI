...
export const DEMO_SKILL_CARDS: SkillCard[] = [
  ...
  },
  {
    ...
    colorTone: 'purple',
    evidenceQuote: '演示能力卡，用于验证 AI 产品问题归因。',
  ...
  },
  {
    ...
    colorTone: 'rose',
    evidenceQuote: '演示能力卡，用于说明任务中的交互判断。',
    sourceRefs: ['CoachAgent 固定任务库'],
    claimLevel: 'hypothesis',
    evidenceType: 'inference',
    pendingVerification: true,
  ...
  citations: [
    {
      ...
      source_note: '演示模式固定引用，用于展示信息结构，不代表具体企业结论。',
    },
  ],
  notice: '当前为演示模式，内容使用固定示例数据，不调用 Qwen，也不写入真实画像。',
};
...
export function createDemoTrialEvaluation(task: ApiTrialTaskDefinition): ApiTrialEvaluation {
  return {
    summary: '演示作答完成了 Bad Case 分层、Top 2 选择、证据引用和验证计划，并在新增约束下调整了验证顺序。',
...
export const DEMO_PROFILE_EVIDENCE: ApiProfileEvidence[] = [
  {
    ...
    observed_evidence: {
      ...
      caveats: ['演示模式使用固定数据', '能力等级仅用于展示评价结构'],
