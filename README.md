...
- 演示模式：与正式模式使用相同页面、文案、按钮和操作流程。个人简历、项目补充材料、候选证据、职业建议、任务材料、五步作答和评价结果已经预填，适合现场完整演示。三轮能力应用仍从 0/3 开始，五步任务仍从 0/5 开始，需要按正式流程完成操作。
- 正式模式：从建档授权和材料上传开始，使用用户材料、后端会话、本地 RAG 与百炼 Qwen 完成完整流程。材料补问、候选证据生成、职业建议、Coach 和任务评价只在用户执行对应操作时调用。

两种模式分别保存建档授权、材料状态、档案助手对话、任务步骤和会话标识，切换模式不会覆盖另一种模式的进度。界面层面的区别仅为演示模式已经预填内容；演示模式仍需启动后端读取比赛资料中的固定任务定义。预填内容本身不会产生付费模型调用，用户主动回复档案助手时会调用一次 Qwen。
...
打开 <http://localhost:3000>，进入“认识自己”，阅读证据档案说明并选择开始建档。正式模式需要上传一份包含可复制文本的 PDF 简历和一份项目补充材料；演示模式已经装入对应示例材料。

01 只保留一个固定对话栏。档案助手仅针对材料中缺失的本人职责、判断依据、限制条件、协作过程和实际结果进行少量补问。用户点击“发送回复”后，前端经后端 `POST /api/v1/profile/exploration/messages` 调用 Qwen；相同材料与对话会复用后端缓存。

两类材料齐备后点击“生成候选证据卡”。前端会请求 `POST /api/v1/profile/proposals`，生成候选能力卡和候选项目经历卡。用户可以确认、修改、合并或删除候选内容，确认后的卡片仍可撤回。只有确认保存的能力卡可以进入职业推荐，未经确认的候选内容不会被后续阶段读取。

材料上传会先调用 `POST /api/v1/profile/materials/extract` 提取可复制文本。个人简历仅接受文本型 PDF；项目补充材料支持 PDF、Word、Markdown 和 TXT。扫描件 OCR 和外部链接抓取暂不支持。
