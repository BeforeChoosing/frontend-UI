...
正式模式下，前端请求自动携带 `X-App-Mode: use` 和客户端请求标识；点击、表单提交、控件变更等操作发送到后端审计接口，后端同时记录接口状态与延迟。演示模式不会写入正式审计日志。审计数据不包含输入正文、附件内容或 API 密钥，可通过后端 `/api/v1/audit/usage` 查看调用量与延迟摘要。
