...
第三阶段分为两个阶段。第一阶段针对已选任务出牌：从已确认能力卡中选择 1–4 张，记录准备如何使用这些能力和本次待验证假设；出牌记录只代表任务前预期，不直接形成评分。第二阶段进入固定三栏工作台：左侧查看并引用任务库材料，中间完成五步微型工作交付物，右侧按需使用三级 Coach 提示。出牌、作答、引用、修改和 Coach 使用都会保存到后端，本机刷新后可以续接。

页面会在浏览器本机保存当前模块、已选任务、02 的能力卡组合、03 的当前阶段和五步位置；任务作答本身由后端会话保存。刷新页面后会直接恢复到未完成位置。相同卡组再次请求职业建议时，后端复用已经校验的结果；任务提交和评价同样具备重复请求保护。
