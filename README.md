...
正式模式下，前端请求自动携带 `X-App-Mode: use`、客户端请求标识和登录会话令牌；点击、表单提交、控件变更等操作发送到后端审计接口，后端按用户记录接口状态、延迟和业务动作。正式版经历对话会在本机后端保存完整聊天记录；上传材料的元数据写入每个账号独立的 `profile_uploaded_materials` 表，原始二进制持久化在服务器账号专属文件目录（单文件 20MB、每账号最多 50 个文件、总量 200MB，按 SHA-256 去重），不会在上传完成后立即删除。演示模式不会写入正式审计日志。审计数据不包含百炼密钥，可通过后端 `/api/v1/audit/usage` 和 `/api/v1/audit/events` 查看当前账号的操作链路。
